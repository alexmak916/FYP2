...
# PATH
MODEL_PATH = './saved_AV_models/AVmodel-2p-100-0.43031.h5'	# 234K
MODEL_PATH_FACENET = '../pretrain_model/FaceNet_keras/facenet_keras.h5'

# Prediction output location
dir_path_pred = './pred/'
dir_path_face_embs = '../../data/video/face_emb/'
...
PART_ONE_RANGE = (193, 196)
PART_TWO_RANGE = (297, 300)
...
def run_predict(part_one=PART_ONE_RANGE, part_two=PART_TWO_RANGE):
	...
	for i, j in zip(range(part_one[0], part_one[1]), range(part_two[0], part_two[1])):
		try:
			...
			try:
				face_embs[0,:,:,:,0] = np.load(dir_path_face_embs+"%05d_face_emb.npy"%i)
				face_embs[0,:,:,:,1] = np.load(dir_path_face_embs+"%05d_face_emb.npy"%j)
				print("%05d_face_emb.npy loaded"%i)
				print("%05d_face_emb.npy loaded"%j)
			except Exception as e:
				print('No face embedding for speaker', i, ' or ', j, "\n", e)

			print("===== Completed processing face_emb =====")
